...
-(int) findHMSoftPeripherals:(int)timeout
{
    ...
    [manager scanForPeripheralsWithServices:@[[CBUUID UUIDWithString:@"F5FD"]] options:options]; // start Scanning
...
-(void) connect:(CBPeripheral *)peripheral
{
    if (!(peripheral.state == CBPeripheralStateConnected)) {
        ...
        //[manager connectPeripheral:peripheral options: @{CBConnectPeripheralOptionNotifyOnConnectionKey: @YES,CBConnectPeripheralOptionNotifyOnDisconnectionKey: @YES,CBConnectPeripheralOptionNotifyOnNotificationKey: @YES, }];
    }
}
...
{
    NSLog(@"RSSI!!:%ld", (long) [RSSI intValue]);
    NSInteger rssi = [RSSI intValue];
    if(rssi < -98 || rssi >= 0){
        return;
    }
...
-(void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral
{
    ...
    //[activePeripheral discoverServices:@[[CBUUID UUIDWithString:@"F5FD"]]];
